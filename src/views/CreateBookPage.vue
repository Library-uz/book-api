...
<template>
    <div class=" inset-0 bg-black/30 grid place-content-center">
        <div class="p-10 rounded bg-white flex flex-col gap-5 sm:w-150">
            ...
            <FormInput v-model="name" :error-message="!!errors.name" :label-name="t('bookName')"
                       :placeholder="t('phBookName')"/>
            <FormInput v-model="description" :error-message="!!errors.description" :label-name="t('description')"
            ...
            <div class="flex flex-col sm:w-130">
                ...
                <textarea v-model="text" class="border rounded h-30"></textarea>
            </div>
            ...
            <div class="flex flex-col sm:w-130">
                ...
                <select v-model="category" class="border rounded py-2.5">
                    <option disabled selected value="">{{ t('select') }}</option>
                    <option v-for="category of getCategoryStore.get.all" :key="category.id" :value="category['@id']">
                        {{ category.name }}
                    </option>
                </select>
            </div>
